...
export default function ExpensePage({ customers, suppliers, documents, showToast, initialPeriod, initialFocus }: ExpensePageProps) {
  ...
  return (
    <div className="expense-page">
      ...
      <div className="expense-mobile-list" aria-label="รายการค่าใช้จ่ายตามตัวกรอง">
        {filtered.map((row) => {
          const document = documentById.get(row.source_document_id);
          const customer = customerById.get(row.customer_id);
          const supplier = supplierById.get(row.supplier_id);
          const linkedName = document?.docNo || document?.doc_no || customer?.name || supplier?.name || "ไม่เชื่อมโยง";
          return (
            <article className={`expense-mobile-card ${row.voided_at ? "is-voided" : row.archived_at ? "is-archived" : ""}`} key={row.id}>
              <header>
                <div><strong>{row.expense_no}</strong><span>{compactDate(row.expense_date)}</span></div>
                <strong className="expense-mobile-total">฿{money(row.total_amount)}</strong>
              </header>
              <div className="expense-mobile-description">{row.description}</div>
              {row.reference && <div className="expense-mobile-reference">อ้างอิง: {row.reference}</div>}
              <div className="expense-mobile-meta">
                <span>{categoryById.get(row.category_id)?.name || "ไม่พบหมวด"}</span>
                <b className={`class-${row.expense_class}`}>{row.expense_class === "direct" ? "Direct" : "Operating"}</b>
                <b className={`status-${row.payment_status}`}>{row.payment_status === "paid" ? "ชำระแล้ว" : "ยังไม่ชำระ"}</b>
              </div>
              <div className="expense-mobile-link"><span>เชื่อมโยง</span><strong>{linkedName}</strong></div>
              <div className="expense-mobile-footer">
                <span>VAT ฿{money(row.vat_amount)}{row.voided_at ? " · ยกเลิก" : row.archived_at ? " · เก็บถาวร" : ""}</span>
                <div className="row-actions">
                  <button type="button" title="ประวัติการแก้ไข" aria-label="ประวัติการแก้ไข" aria-expanded={historyId === row.id} onClick={() => setHistoryId(historyId === row.id ? null : row.id)}><History size={17} /></button>
                  <button type="button" title="แก้ไข" aria-label="แก้ไข" onClick={() => openEditor(row)}><Pencil size={17} /></button>
                  {!row.voided_at && <button type="button" title={row.archived_at ? "นำกลับมาใช้" : "เก็บถาวร"} aria-label={row.archived_at ? "นำกลับมาใช้" : "เก็บถาวร"} onClick={() => void transitionExpense(row, row.archived_at ? "restore" : "archive")}>{row.archived_at ? <ArchiveRestore size={17} /> : <Archive size={17} />}</button>}
                  {!row.voided_at && <button type="button" title="ยกเลิกรายการ" aria-label="ยกเลิกรายการ" className="danger" onClick={() => void transitionExpense(row, "void")}><X size={17} /></button>}
                </div>
              </div>
            </article>
          );
      ...
      <style jsx>{`
        ...
        .expense-mobile-list{display:none}.expense-mobile-card{border:1px solid var(--expense-line);border-radius:8px;background:#111923;padding:14px;color:var(--expense-text)}.expense-mobile-card>header,.expense-mobile-footer{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.expense-mobile-card>header>div{display:grid;gap:3px;min-width:0}.expense-mobile-card>header span,.expense-mobile-reference,.expense-mobile-footer>span{color:var(--expense-muted);font-size:12px}.expense-mobile-total{color:#f8fafc;font-size:18px;white-space:nowrap}.expense-mobile-description{margin-top:12px;color:#f8fafc;font-size:15px;font-weight:800;overflow-wrap:anywhere}.expense-mobile-reference{margin-top:4px;overflow-wrap:anywhere}.expense-mobile-meta{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:10px}.expense-mobile-meta>span{color:#dbe3ef;font-size:12px}.expense-mobile-link{display:grid;grid-template-columns:auto minmax(0,1fr);gap:10px;margin-top:12px;padding:10px;border-radius:6px;background:#0b111a;font-size:12px}.expense-mobile-link span{color:var(--expense-muted)}.expense-mobile-link strong{text-align:right;overflow-wrap:anywhere}.expense-mobile-footer{align-items:center;margin-top:12px;padding-top:12px;border-top:1px solid var(--expense-line)}.expense-mobile-card.is-voided{border-color:rgba(239,68,68,.4)}.expense-mobile-card.is-archived{border-color:rgba(148,163,184,.35)}
