      ...
      <div className="marketing-shell-grid" style={{ display: "grid", gridTemplateColumns: "250px minmax(0,1fr)" }}>
        <aside className="marketing-sidebar" style={{ background: "rgba(7,10,13,0.92)", borderRight: "1px solid rgba(255,107,0,0.16)", padding: 22, display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ marginTop: "auto", borderRadius: 18, padding: 18, background: "linear-gradient(180deg,rgba(255,107,0,0.14),rgba(255,107,0,0.04))", border: "1px solid rgba(255,107,0,0.28)" }}>
            ...
            <button type="button" onClick={() => showToast("Marketing dashboard พร้อมใช้งาน")} style={{ width: "100%", border: 0, borderRadius: 12, padding: "10px 12px", color: "#fff", fontWeight: 900, background: "linear-gradient(135deg,#FF6B00,#EA580C)" }}>Check Setup</button>
          </div>
        </aside>
    ...
    <div style={{ display: "grid", gap: 16, marginBottom: 22 }}>
      <div style={{ ...cardStyle, display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", flexWrap: "wrap", background: "linear-gradient(135deg,#151B26 0%,#121720 56%,rgba(255,107,0,0.14) 100%)" }}>
        <div>
          ...
          <h2 style={{ fontSize: 24, fontWeight: 900, marginTop: 6 }}>Dashboard วิเคราะห์ลูกค้า</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(320px,1fr))", gap: 16 }}>
        <div style={cardStyle}>
          <h2 style={{ fontSize: 18, fontWeight: 900, marginBottom: 4 }}>ลูกค้าเจอเราจากไหน</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(320px,1fr))", gap: 16 }}>
        <div style={cardStyle}>
          <h2 style={{ fontSize: 18, fontWeight: 900, marginBottom: 4 }}>ประเภทธุรกิจลูกค้า</h2>
        ...
        <div style={cardStyle}>
          <h2 style={{ fontSize: 18, fontWeight: 900, marginBottom: 4 }}>สินค้า / บริการยอดนิยม</h2>
  ...
  return (
    <div className="rich-editor-shell" style={{ border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, overflow: "hidden", background: "#1A2233" }}>
      ...
      <div className="rich-editor-toolbar" style={{ display: "flex", flexWrap: "wrap", gap: 4, padding: "8px 10px", borderBottom: "1px solid rgba(255,255,255,0.08)", background: "#141A24", alignItems: "center" }}>
        ...
        <div style={divider} />
        {/* Text style */}
        <button type="button" aria-label="Bold" style={btnStyle()} onClick={() => exec("bold")} title="หนา"><b>B</b></button>
        <button type="button" aria-label="Italic" style={btnStyle()} onClick={() => exec("italic")} title="เอียง"><i>I</i></button>
        <button type="button" aria-label="Underline" style={btnStyle()} onClick={() => exec("underline")} title="ขีดเส้นใต้"><u>U</u></button>
        <button type="button" aria-label="Strikethrough" style={btnStyle()} onClick={() => exec("strikeThrough")} title="ขีดทับ"><s>S</s></button>
        <div style={divider} />
        {/* Color */}
        ...
        <div style={divider} />
        {/* Align */}
        <button type="button" aria-label="Align left" style={btnStyle()} onClick={() => exec("justifyLeft")} title="ชิดซ้าย">⬅</button>
        <button type="button" aria-label="Align center" style={btnStyle()} onClick={() => exec("justifyCenter")} title="กึ่งกลาง">≡</button>
        <button type="button" aria-label="Align right" style={btnStyle()} onClick={() => exec("justifyRight")} title="ชิดขวา">➡</button>
        <div style={divider} />
        {/* List */}
        <button type="button" aria-label="Bulleted list" style={btnStyle()} onClick={() => exec("insertUnorderedList")} title="รายการ">• ≡</button>
        <button type="button" aria-label="Numbered list" style={btnStyle()} onClick={() => exec("insertOrderedList")} title="รายการตัวเลข">1. ≡</button>
        <div style={divider} />
        {/* Link */}
        ...
        <div style={divider} />
        {/* Undo/Redo */}
        <button type="button" aria-label="Undo" style={btnStyle()} onClick={() => exec("undo")} title="ย้อนกลับ">↩</button>
        <button type="button" aria-label="Redo" style={btnStyle()} onClick={() => exec("redo")} title="ทำซ้ำ">↪</button>
        ...
        {filtered.map(p => (
          <div key={p.id} className="cms-blog-row" style={{ background: "#141A24", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "16px 20px", display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div className="cms-blog-copy" style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 12, color: "#94A3B8" }}>{p.category} · {fmtDate(p.date)}</div>
            ...
            <div className="cms-blog-actions" style={{ display: "flex", gap: 6, flexShrink: 0 }}>
              <CIconBtn aria-label={`Edit article: ${p.title}`} onClick={() => setEditing({ ...p })}>✏️</CIconBtn>
              <CIconBtn aria-label={`Delete article: ${p.title}`} onClick={() => del(p.id)} danger>🗑️</CIconBtn>
            </div>
          </div>
        ))}
        ...
        <CModal title={editing.id ? "แก้ไขรีวิว" : "เพิ่มรีวิว"} onClose={() => setEditing(null)} width={480}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <CField label="ดาว (1-5)">
              <div style={{ display: "flex", gap: 6 }}>
                {[1,2,3,4,5].map(s => (
                  <button type="button" key={s} onClick={() => setEditing(p => ({ ...p, stars: s }))} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 24, color: s <= editing.stars ? "#F59E0B" : "#333" }}>★</button>
                ))}
              </div>
            </CField>
