...
import {
  Plus, Minus, Check, Phone, MessageCircle, ArrowRight, ChevronRight, Menu, X, FileText, Calculator, CheckCircle2, Cog, Home, PackageCheck, Send, CheckCircle, Upload, Weight, Zap, MonitorUp, ShieldCheck, MapPin, Mail } from "lucide-react";
...
export default function RollUpLandingPage() {
  ...
  const serviceText = (field: string, fallback: string) => cmsValue(cms, `servicesDetail.rollup.${field}`, fallback);
  ...
  return (
    <div className="brand-interior brand-service-detail min-h-screen font-['Prompt',sans-serif] text-white bg-[#050806]">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10 w-full pt-16 pb-24">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="max-w-3xl">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold mb-6 border border-[#FF7A00]/30 text-[#FF7A00] bg-transparent">
              {serviceText("eyebrow", "บริการพิมพ์และจัดจำหน่ายโครง")}
            </div>

            <h1 className="font-['Kanit'] font-extrabold text-5xl lg:text-7xl mb-6 leading-tight text-white">
              {serviceText("title", "Roll Up")}
              <span className="block text-[#FF7A00]">{serviceText("highlight", "/ X-Stand")}</span>
            </h1>

            <p className="text-base text-gray-300 max-w-xl mb-10 leading-relaxed">
              {serviceText("subtitle", "ป้ายตั้งพื้นเคลื่อนที่ ติดตั้งง่ายภายใน 1 นาที มาพร้อมกระเป๋าพกพาสะดวก เหมาะสำหรับงานออกบูธ นิทรรศการ และป้ายส่งเสริมการขายหน้าร้าน พิมพ์สีคมชัดโดดเด่น")}
            </p>
      ...
      <section id="materials" className="py-24 bg-[#050806]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="mb-12 text-center max-w-3xl mx-auto">
      ...
      {/* ── 9. FOOTER ── */}
      <SharedServiceSections serviceKey="rollup" />
      <GlobalFooter />
    </div>
  );
}
